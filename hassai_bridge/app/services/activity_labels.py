...
def _try_json(text: str) -> Any | None:
    ...
    try:
        ...
        # Sometimes tools wrap JSON with a header line
        start = raw.find("{")
        if start < 0:
            return None
        try:
...
def tool_result_preview(name: str, content: str | None, *, max_len: int = 100) -> str:
    """Short outcome line for a completed tool step."""
    ...
    if name == "ha_explain_event":
        ...
        if isinstance(data, dict):
            ...
            if summary:
                return clip_detail(summary, max_len)

    if name in {"ha_list_entities", "ha_list_entity_registry", "ha_list_devices", "ha_list_areas"}:
        m = _SHOWING_RE.search(text)
        if m:
            return clip_detail(f"{m.group(1)} found", max_len)
        lines = [ln for ln in text.splitlines() if ln.strip() and not ln.strip().lower().startswith("showing")]
        if lines:
            return clip_detail(f"{len(lines)} row(s) · {_first_line(text)}", max_len)

    if name == "ha_get_state":
        # Prefer "entity_id: state" style first line
        ...
        return clip_detail(first, max_len)

    if name == "ha_get_logs":
        lines = [ln for ln in text.splitlines() if ln.strip() and not ln.startswith("#")]
        if not lines:
            return clip_detail("no matching lines", max_len)
        return clip_detail(f"{len(lines)} line(s) · {_first_line(lines[-1])}", max_len)

    if name == "ha_call_service":
        first = _first_line(text)
        if first.lower().startswith("ok") or "called" in first.lower():
            return clip_detail(first, max_len)
        return clip_detail(first, max_len)
    ...
    if name == "search_web":
        ...
        if m:
            return clip_detail(f"{m.group(1)} results", max_len)
    ...
    if name == "background_tasks":
        ...
        if isinstance(data, dict):
            ...
            bits = [b for b in (status or "ok", title, tid) if b]
            return clip_detail(" · ".join(bits), max_len)
        return clip_detail(_first_line(text), max_len)
