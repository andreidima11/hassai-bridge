...
let frameHeightBackup = null;

function visibleIframeHeightPx() {
  const frame = haFrame();
  if (!frame || !window.parent || window.parent === window) return 0;
  let pvv;
  try {
  ...
  if (!pvv || !Number.isFinite(pvv.height)) return 0;
  const top = frame.getBoundingClientRect().top;
  const h = Math.round(pvv.offsetTop + pvv.height - top);
  const parentH = window.parent.innerHeight || 0;
  const maxH = Math.round(parentH - Math.max(0, top));
  if (h < 180 || maxH < 180) return 0;
  return Math.max(180, Math.min(h, maxH));
}

function fitIframeToKeyboard() {
  const frame = haFrame();
  if (!frame) return false;
  const h = visibleIframeHeightPx();
  if (!h) return false;
  if (frameHeightBackup == null) frameHeightBackup = frame.style.height;
  frame.style.height = `${h}px`;
  frame.style.maxHeight = `${h}px`;
  try { window.parent.scrollTo(0, 0); } catch (_) { /* ignore */ }
  return true;
}

function releaseIframe() {
  const frame = haFrame();
  if (!frame || frameHeightBackup == null) return;
  frame.style.height = frameHeightBackup;
  frame.style.maxHeight = "";
  frameHeightBackup = null;
}

function onComposerFocus() {
  const fitted = fitIframeToKeyboard();
  document.body.classList.toggle("chat-kb-dock", !fitted);
}

function onComposerBlur() {
  document.body.classList.remove("chat-kb-dock");
  setTimeout(() => {
...
try {
  window.parent?.visualViewport?.addEventListener("resize", () => {
    if (document.activeElement === inputEl) fitIframeToKeyboard();
  });
} catch (_) { /* ignore */ }
