    ...
    <div ref={rootRef} className="theme-select relative">
      ...
      {open ? (
        <ul
          ...
          role="listbox"
          aria-label={ariaLabel}
          className="theme-select__menu absolute left-0 right-0 top-[calc(100%+6px)] z-[60] max-h-52 overflow-y-auto rounded-xl border border-white/10 bg-card p-1 shadow-float"
        >
          {options.map((row) => {
