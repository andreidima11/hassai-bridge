...
def test_build_empty_recs_prefers_live_actions_over_llm_status():
    ...
    async def _run():
        with patch.object(recs, "enabled", return_value=True), patch(
            ...
            recs.rl, "recs_mode", return_value="medium"
        ), patch.object(recs.rl, "ensure_empty_pool", new=AsyncMock(return_value=llm)):
            return await recs.build_empty_recs(lang="ro", habits=habits, atmosphere={}, limit=5)
    ...
    assert "Închide Poarta auto" in blob
    assert "Deschide Poarta pietonala" in blob
    assert any("Aprinde Lumini" in x or "Aprinde Lampa" in x or "Aprinde LED" in x for x in labels)
    assert sum(1 for c in chips if c["kind"] == "action") >= 3
